...
export class App {
    ...
    updateLocation(bikeId: string, state: string, city: string, latitude: number, longitude: number) {
        const bike = this.checkBikeExists(bikeId);
    ...
    checkBikeExists(bikeId: string): Bike {
        const bike = this.bikes.find((bike) => bike.id === bikeId);
        if (!bike) {
          throw new Error("Bike not found.");
        }
